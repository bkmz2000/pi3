...
function checkAccess(projectId: string, userId: string, requiredRole: 'owner' | 'editor' | 'viewer' | 'write'): { allowed: boolean; role: string | null } {
  const db = getDb();
...
router.get('/', (req: Request, res: Response): void => {
  ...
  const access = checkAccess(projectId, req.user!.id, 'viewer');
  if (!access.allowed) {
...
router.post('/', (req: Request, res: Response): void => {
  ...
  const access = checkAccess(projectId, req.user!.id, 'editor');
  if (!access.allowed) {
    res.status(access.role === null ? 404 : 403).json({
      error: access.role === null ? 'Not Found' : 'Forbidden',
