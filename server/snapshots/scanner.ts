...
// Patterns kept intentionally simple and readable — the goal is a lint-level
// catch, not a filter. False positives are acceptable (human review resolves)
// and false negatives are expected (that's what the takedown flow is for).
const EMAIL_RE = /[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/g;
const PHONE_RE = /(?:\+?\d[\s\-().]?){7,15}\d/g; // 7–15 digits with common separators
...
function collect(text: string, where: string, out: ScanFinding[]): void {
  ...
  for (const m of text.matchAll(PHONE_RE)) {
    // Require at least 7 digits (already enforced by the pattern) and skip
    // matches that are almost entirely separators, to reduce noise.
...
export function scanSnapshot(input: {
  ...
  if (input.assets) {
    ...
    try {
      ...
      // ignore serialization errors — those aren't the scanner's problem
    }
  }
