...
function SettingsPanel({
  ...
  const showActorInfo = useIde((s) => s.showActorInfo);
  const setShowActorInfo = useIde((s) => s.setShowActorInfo);
  ...
  return (
    <>
      ...
      <div style={{ padding: "16px", overflowY: "auto", flex: 1 }}>
        ...
        />
        <ToggleRow
          label={t('sideMenu.showActorInfo')}
          hint={t('sideMenu.showActorInfoHint')}
          on={showActorInfo}
          theme={theme}
          accent={theme.accent}
          onChange={(v) => setShowActorInfo(v)}
        />
      </div>
    </>
  );
