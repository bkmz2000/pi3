...
state = State(won=False, layer=None)
...
def tick():
    if state.won and Keyboard.r.pressed:
        make_room()
        return
    ...
    if not state.won:
        ...
        if state.layer.tile_at(hero.x + vx * speed, hero.y) != "wall":
            hero.vx = vx * speed
        else:
            hero.vx = 0
        if state.layer.tile_at(hero.x, hero.y + vy * speed) != "wall":
            hero.vy = vy * speed
        else:
            hero.vy = 0
        ...
        if state.layer.tile_at(hero.x, hero.y) == "exit":
            state.won = True

    background(Colors.black)
    no_stroke()
    for c, r, name in state.layer.tiles():
