...
export const CONCEPTS: DocConcept[] = [
  {
    ...
    ru: {
      ...
      body:
        "Ресурс — это то, что ты загружаешь в проект через панель «Ресурсы»: спрайт, анимация, тайловая карта или звук. После загрузки ресурс доступен из кода через глобальное пространство имён `assets` по имени, которое ты ему дал.",
    },
    example:
  ...
  },
  {
    ...
    ru: {
      ...
      body:
        "Холст использует пиксельные координаты. По умолчанию (0, 0) — это верхний левый угол; x растёт вправо, y растёт вниз. У большинства объектов есть «якорь» — точка фигуры, которая совмещается с переданными координатами (x, y). У фигур это обычно центр; у изображений — верхний левый угол, если не изменишь.",
    },
  ...
  },
  {
    ...
    ru: {
      title: "Игровой цикл",
      body:
        "Игра — это цикл: много раз в секунду программа очищает экран, обновляет состояние (позиции, таймеры, ввод) и рисует новый кадр. Передай в `run()` функцию, которую хочешь выполнять примерно 60 раз в секунду. Обычно эту функцию называют `main` (главная), а из неё в нужном порядке вызывают другие функции, каждая из которых отвечает за свою область: рисование, управление, физика и т.д. Переменные, которые нужно сохранять между кадрами, живут вне функции.",
    },
    example:
      "x = 0\n\ndef frame():\n    global x\n    x += 1\n    background(\"black\")\n    circle(x, 100, 20)\n\nrun(frame)",
  },
  ...
  },
  {
    ...
    ru: {
      ...
      body:
        "Анимация — это последовательность кадров. Воспроизведение — это переход между кадрами по времени. Частота кадров (fps) задаёт скорость. Циклическая анимация возвращается к началу; не циклическая останавливается на последнем кадре. Чтобы анимация шла, вызывай `.update()` в каждом кадре.",
    },
    example:
  ...
  },
  {
    ...
    ru: {
      ...
      body:
        "Столкновение — это перекрытие двух фигур. Самая простая проверка — касаются ли два хитбокса (прямоугольника вокруг фигуры). У актёров это уже встроено через `Collider`. Обнаружение только сообщает, что столкновение произошло — чтобы актёр остановился, столкновение нужно ещё и «разрешить» (вытолкнуть наружу).",
    },
    example:
  ...
  },
  {
    ...
    ru: {
      ...
      body:
        "`translate`, `rotate` и `scale` меняют систему координат для всего, что рисуется после них. Они складываются стеком: `push()` сохраняет текущую трансформацию, `pop()` восстанавливает её. Порядок важен — `translate` потом `rotate` поворачивает вокруг новой точки отсчёта, а не вокруг (0, 0) холста. Оборачивай локальную трансформацию в push/pop, чтобы она не «протекла» на другие рисунки.",
    },
    example:
