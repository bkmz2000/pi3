    ...
    entries: [
      ...
      },
      {
        ...
        params: [
          ...
          { name: "pos()", type: "Vector2", en: "Position as a fresh Vector2 computed from x/y. Set with set_pos(v). Useful for math: actor.set_pos(actor.pos() + Vector2(0, 1)).", ru: "Позиция как новый Vector2, вычисленный из x/y. Устанавливается через set_pos(v). Удобно для математики: actor.set_pos(actor.pos() + Vector2(0, 1))." },
          { name: "set_pos(v)", type: "—", en: "Set position from a Vector2 or (x, y) pair.", ru: "Устанавливает позицию из Vector2 или пары (x, y)." },
          { name: "vel()", type: "Vector2", en: "Velocity as a fresh Vector2 computed from vx/vy. Set with set_vel(v). Build one with Polar(speed, angle) for movement at an angle: bullet.set_vel(Polar(8, ship.angle)).", ru: "Скорость как новый Vector2, вычисленный из vx/vy. Устанавливается через set_vel(v). Используйте Polar(speed, angle) для движения под углом: bullet.set_vel(Polar(8, ship.angle))." },
