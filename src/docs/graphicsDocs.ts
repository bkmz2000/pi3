  ...
  // ─── Tilemap ───────────────────────────────────────────────────────────────
  {
    ...
    entries: [
      ...
      },
      {
        ...
        en: 'Tag tiles by name, then query a Group of read-only tile colliders for every cell whose tile-name has that tag. Tags are set in Python at run time (no persistence yet); the returned Group works with collides_any() and as light obstacles. TileMap.tag delegates to every layer containing the tile; TileMap.all_tiles aggregates across layers. TilemapLayer.tag / .all_tiles do the same on a single layer. Returns a Group of TileRef actors centered on each matching cell, sized to tile_size. The Group is cached until tags change.',
        ru: 'Пометьте тайлы по имени, затем получите Group коллайдеров для всех ячеек с этой меткой. Метки задаются в Python во время выполнения (постоянство пока не реализовано); возвращаемая Group работает с collides_any() и подходит как препятствие для света. TileMap.tag делегирует во все слои, где встречается тайл; TileMap.all_tiles агрегирует по слоям. TilemapLayer.tag / .all_tiles делают то же самое на одном слое. Возвращает Group объектов TileRef в центре каждой ячейки, размера tile_size. Группа кэшируется до изменения меток.',
        params: [
