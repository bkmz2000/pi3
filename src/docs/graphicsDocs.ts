  ...
  // ─── Math ──────────────────────────────────────────────────────────────────
  {
    ...
    entries: [
      ...
      },
      {
        ...
        en: "Build a Vector2 from a magnitude and an angle in degrees. Angle convention matches actor.angle and actor.move(): 0° = north (up), 90° = east (+x), 180° = south, 270° = west — clockwise like a compass. The most common use is setting actor.vel for continuous motion at an angle without writing the trig yourself.",
        ru: "Создаёт Vector2 из модуля и угла в градусах. Соглашение об угле совпадает с actor.angle и actor.move(): 0° = север (вверх), 90° = восток (+x), 180° = юг, 270° = запад — по часовой стрелке, как компас. Чаще всего применяется для задания actor.vel — непрерывное движение под углом без ручной тригонометрии.",
    ...
    entries: [
      {
        ...
        params: [
          ...
          { name: "angle", type: "number", optional: true, default: "0", en: "Visual rotation in degrees, clockwise from north: 0° = up, 90° = right, 180° = down. Affects draw() and actor.move(distance). For continuous motion at an angle, set actor.vel = Polar(speed, angle).", ru: "Визуальный угол поворота в градусах, по часовой стрелке от севера: 0° = вверх, 90° = вправо, 180° = вниз. Влияет на отрисовку и actor.move(distance). Для непрерывного движения под углом задайте actor.vel = Polar(speed, angle)." },
      ...
      },
      {
        ...
        params: [
          { name: "move(distance)", type: "—", en: "One-shot step: move forward by distance in the direction of angle (0° = up, 90° = right). Useful when you want a per-frame imperative step instead of a continuous velocity. For continuous motion at an angle, set actor.vel = Polar(distance_per_frame, angle).", ru: "Одношаговое движение: сместиться вперёд на distance в направлении угла (0° = вверх, 90° = вправо). Подходит, когда нужен императивный шаг каждый кадр, а не постоянная скорость. Для непрерывного движения под углом задайте actor.vel = Polar(скорость_за_кадр, angle)." },
