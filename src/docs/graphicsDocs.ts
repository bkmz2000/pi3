  ...
  // ─── Utilities ─────────────────────────────────────────────────────────────
  {
    ...
    entries: [
      ...
      },
      {
        ...
        en: "Random integer between a and b, inclusive on both ends. Use this instead of random() when you need a whole number, like a dice roll or a grid index.",
        ru: "Случайное целое число между a и b, включая обе границы. Используй вместо random(), когда нужно целое число — например, бросок кубика или индекс в сетке.",
  ...
  // ─── Debug (pi3.debug) ─────────────────────────────────────────────────────
  {
    ...
    entries: [
      ...
      },
      {
        ...
        en: "Register a stack (top = last element) for the debug scrubber. Same fill/stroke selector syntax as debug.array — an index, debug.range, debug.singles, or debug.named — but indices count from the bottom, so the top of the stack is always index len(data) - 1.",
        ru: "Регистрирует стек (вершина = последний элемент) для отладочной ленты. Те же селекторы заливки/обводки, что у debug.array — индекс, debug.range, debug.singles или debug.named — но индексы считаются от дна, поэтому вершина стека всегда имеет индекс len(data) - 1.",
      ...
      },
      {
        id: "debug_set",
        name: "debug.set",
        signature: "debug.set(data, red=..., stroke_blue=..., stroke_width=2)",
        ...
        example: "from pi3 import debug\n\nseen = {3, 1, 4, 1, 5}\ndebug.set(seen, green=0)  # highlights the smallest element (sorted order)\ndebug.show()",
        advanced: {
          en: "Because rendering requires sorted(data), every element must be mutually comparable — a set mixing ints and strings raises a TypeError here even though the set itself is valid Python. If a student's set holds unsortable/mixed items, convert to a sorted list of a display key before calling debug.set.",
          ru: "Так как отрисовка требует sorted(data), все элементы должны быть взаимно сравнимы — множество, смешивающее int и str, вызовет TypeError здесь, даже если само множество — валидный Python. Если множество ученика содержит несравнимые/смешанные элементы, преобразуй его в отсортированный список по ключу отображения перед debug.set.",
        },
        params: [
          ...
          { name: "red / green / blue / yellow / cyan / gray", type: "int | debug.range | debug.singles | list | debug.named", optional: true, en: "Fill this cell (by position in sorted order) with this color.", ru: "Залить ячейку (по позиции в отсортированном порядке) этим цветом." },
          { name: "stroke_red / stroke_green / ...", type: "same as above", optional: true, en: "Draw a border of this color around the cell.", ru: "Нарисовать вокруг ячейки рамку этого цвета." },
          { name: "stroke_width", type: "int", optional: true, default: "2", en: "Border thickness in pixels for all strokes in this call.", ru: "Толщина рамки в пикселях для всех обводок этого вызова." },
        ],
      },
