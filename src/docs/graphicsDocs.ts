    ...
    entries: [
      ...
      },
      {
        ...
        params: [
          ...
          { name: 'tag(name, *tags)', type: '—', en: 'Apply one or more tag strings to every layer in the tilemap that contains the named tile. Example: level.tag("brick", "wall").', ru: 'Применяет одну или несколько меток ко всем слоям, содержащим указанный тайл. Пример: level.tag("brick", "wall").' },
          { name: 'all_tiles(tag)', type: 'Group', en: 'Return a Group of read-only tile colliders for every cell whose tile-name has tag, aggregated across all layers. Use as a collision target or as light obstacles. Example: walls = level.all_tiles("wall"); player.collides_any(walls).', ru: 'Возвращает Group коллайдеров для всех ячеек, чьё имя тайла помечено tag, агрегируя по всем слоям. Подходит как цель столкновений или препятствие для света. Пример: walls = level.all_tiles("wall"); player.collides_any(walls).' },
        ],
      },
      {
        ...
        params: [
          ...
          { name: 'tag(name, *tags)', type: '—', en: 'Associate one or more tag strings with a tile name on this layer. Repeated calls accumulate; duplicates have no effect. Returns self for chaining.', ru: 'Связывает одну или несколько меток с именем тайла на этом слое. Повторные вызовы накапливаются; дубли игнорируются. Возвращает self для цепочечных вызовов.' },
          { name: 'all_tiles(tag)', type: 'Group', en: 'Return a Group of tile colliders for every cell whose tile-name has tag. Each collider is centered on its cell and sized to tile_size. The group is cached until tags change.', ru: 'Возвращает Group коллайдеров для всех ячеек, чьё имя тайла помечено tag. Каждый коллайдер центрирован в ячейке и имеет размер tile_size. Группа кэшируется до изменения меток.' },
  ...
  // ─── Themes & Lighting ─────────────────────────────────────────────────────
  {
    ...
    entries: [
      ...
      },
      {
        ...
        en: "Multiply-blended lighting overlay with optional shadow-casting sources. Add obstacles (walls, tiles) and sources (torches, the player), then call light.draw() last each frame so the lightmap composites over everything else.",
        ru: "Освещение с мультипликативным наложением и поддержкой теней. Добавьте препятствия (стены, тайлы) и источники (факелы, игрока), затем вызывайте light.draw() последним в каждом кадре, чтобы карта света наложилась поверх остального.",
        params: [
